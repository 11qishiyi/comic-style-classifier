...
> ### ⚠️ 先看这一条：外部数据验证结果
>
> 在**训练时从未使用过**的外部数据集上验证（详见仓库 `docs/external-validation.md`）：
>
> | 外部集 | 预期 | 模型 | 判对率 |
> |---|---|---|---|
> | 《钢之炼金术师》分格（文字已抹除，100 张） | japanese | `panel` | **1.000** |
> | Manga109 109 部日漫封面 | japanese | `panel` | **0.862** |
> | Manga109（同上） | japanese | `cover` | **0.101** |
> | Garfield 美国报纸连环画（400 张） | western | `cover` | 0.193 |
> | Garfield（同上） | western | `panel` | 0.323 |
>
> **`cover` 模型的 0.8627 是特定域内的指标**——它学的是「封面设计年代感」
> （现代日系封面 vs 经典美漫封面），不是「民族画风」。
> 在真实的日本漫画封面（Manga109，1970s–2010s 复古设计）上只有 10.1% 判对，
> 因为那些封面在视觉统计上更接近黄金时代美漫。
>
> **`panel` 模型跨来源泛化良好**（1.000 / 0.862），是更可靠的那个。
> 但它的 `western` 类同样偏向经典美漫印刷风格，现代美漫/报纸连环画会判错。
>
> **不要用它们判别现代美漫、报纸连环画或复古日漫单行本封面。**

1. **模型不依赖文字。** 涂白消融：把检测到的文字区域涂成纯白后重新推理，
   panel 从 0.9649 降到 0.9430（−2.2 分），cover 从 0.8627 升到 0.8758（+1.3 分）。
   更有说服力的是自然实验：Garfield（通篇英文）80% 被判成日漫、
   Manga109（日文标题）87% 被判成欧美——**文字语言与预测方向相反**，
   说明模型完全没有在读文字。详见仓库 `docs/text-dependency.md`。
2. **中↔日区分受标注口径限制，不是模型能力问题。**
...
3. **`western` 类偏向经典欧美漫画**（1938–1970 年代），与当代欧美漫画有差距。
4. **跨形态不可用**。用 `panel` 模型判封面只有 0.822，用 `cover` 模型判分格只有 0.807。
   请按输入形态选择对应模型：
   - 单幅封面 / 插画 → `cover`
   - 漫画单格 → `panel`
   - 整页漫画 → 两个都不合适，需先切分格或自行微调
5. **国漫只有封面级数据**，`chinese` 类没有内页样本。
6. 输入图像很小（如 190×247 的缩略图）时置信度会偏低，但仍可用。
